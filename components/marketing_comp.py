...
def run_marketing():
    ...
    with header_col2:
        st.write("🔔 June 8, 2025")
        st.button("🔄 Refresh News")

    # Today's Market Brief
    with st.container(border=True):
        st.subheader("📋 Today's Market Brief")
        st.markdown("- 🔥 OpenAI launches GPT-5 Enterprise with improved reasoning and context handling. **2h ago**")
        st.markdown("- 📈 LinkedIn engagement rates are up 12% across all industries. **4h ago**")
        st.markdown("- 📢 Google Ads introduces AI Max for Search Campaigns. **6h ago**")
        st.markdown("- 📷 Instagram expands Reels to 3 minutes for all users globally. **8h ago**")
        st.markdown("- 🤖 Runway releases new Gen-3 video model with major realism upgrade. **10h ago**")
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            with title_col:
                st.subheader("💡 Opportunities for Agencies")
            with link_col:
                st.markdown("<div style='margin-top:10px;'><a href='#' >View All-></a></div>", unsafe_allow_html=True)
